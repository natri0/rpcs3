 ...
 * - All of the pointers passed are **ALREADY ALLOCATED** by the game.
 * - We do NOT need to allocate anything -- simply write to them our results and let the game handle it.
 */
error_code sceNpClansGetClanList(vm::ptr<SceNpClansRequestHandle> handle, vm::cptr<SceNpClansPagingRequest> paging, vm::ptr<SceNpClansEntry> clanList, vm::ptr<SceNpClansPagingResult> pageResult)
{
	...
	// See: we're making sure the game allocated and assigned the pointers for us
...
error_code sceNpClansUpdateMemberInfo(vm::ptr<SceNpClansRequestHandle> handle, SceNpClanId clanId, vm::cptr<SceNpClansUpdatableMemberInfo> info)
{
	...
	if (!info)
	{
		// TODO: add more checks for info (i.e. lengths)
		return SCE_NP_CLANS_ERROR_INVALID_ARGUMENT;
	}
...
// NOTE: if `clanId` is UINT32_MAX, *make sure the SceneList's clubhouse scene type is set to `Clubhouse`*
